...
public class UpdateCryptoInWalletActivity implements RequestHandler<UpdateCryptoInWalletRequest, UpdateCryptoInWalletResult>  {
    ...
    @Inject
    public UpdateCryptoInWalletActivity(UsersDao usersDao, WalletDao walletDao, CryptoDao cryptoDao) {
    ...
    @Override
    public UpdateCryptoInWalletResult handleRequest(
            final UpdateCryptoInWalletRequest updateCryptoInWalletRequest, Context context) {

        if (updateCryptoInWalletRequest == null) {
        ...
        if (updateCryptoInWalletRequest.getToken() == null) {
            throw new BadRequestException((ErrorMessage.builder()
                    .withStatus(400)
                    .withError("Bad request ...")
                    .withContentType("application/json")
        ...
        // check token
        Users user = jsonWebToken.getUserInformationFromToken(updateCryptoInWalletRequest.getToken());
        ...
        if (!(user.getAdmin() || user.getEmail().equals(updateCryptoInWalletRequest.getUserId()) )) {
        ...
        Wallet findedWallet = walletDao.getWallet(updateCryptoInWalletRequest.getUserId(), updateCryptoInWalletRequest.getWalletName());
        ...
        for (int i = 0; i < cryptocurrencyList.size(); i++) {
            ...
            if (availableCrypto.getCryptoName().equals(updateCryptoInWalletRequest.getCryptoType())) {
                isInAvailable = true;
                break;
            }
        }
        ...
        for (int i = 0; i < cryptocurrencyInWallet.size(); i++) {
            if (cryptocurrencyInWallet.get(i).getCryptoName().equals(updateCryptoInWalletRequest.getCryptoName())) {
        ...
        updateCryptoInWalletRequest.setCryptoCost(updateCryptoInWalletRequest.getCryptoAmount() * availableCrypto.getCryptoCost());
        CryptoCurrenciesModel cryptoModel = new CryptoCurrenciesModel(updateCryptoInWalletRequest.getCryptoName(), updateCryptoInWalletRequest.getCryptoType(),
                updateCryptoInWalletRequest.getImage(), updateCryptoInWalletRequest.getImageUrl(),updateCryptoInWalletRequest.getCryptoDescription(), updateCryptoInWalletRequest.getCryptoAmount(),
                updateCryptoInWalletRequest.getCryptoCost());
        cryptocurrencyInWallet.set(index, cryptoModel);
        findedWallet.setCryptosCount((double) cryptocurrencyInWallet.size());
        findedWallet.setCryptosCost(findedWallet.getCryptosCost() + updateCryptoInWalletRequest.getCryptoCost());
        ...
        return UpdateCryptoInWalletResult.builder()
